...
const conversionComponents = (React, tagName, elementClass, events) => {
  ...
  console.log("咕咕咕咕咕咕过过过", classProps)
  class ReactComponent extends Component {
    constructor(props) {
      super(props);
      console.log("个人分的", props)
      this._initElementProps(props)
      // 保存DOM的引用
      this.DOM = null;
    }
    _initElementProps(props) {
      const propsItems = ['value', 'onChange']
      for (const p in props) {
        // if (!(p in HTMLElement.prototype)) {
          if(propsItems.includes(p)) {
            classProps.add(p);
          }
    ...
    render() {
      ...
      this.DOMProps = {};
      console.log("咕咕咕咕咕咕过过过过this.props", this.props)
      for (const [k, v] of Object.entries(this.props)) {
        if (classProps.has(k)) {
          this.DOMProps[k] = v;
        }
      }
      console.log("个人分的props", props, this.DOMProps)
      return createElement(tagName, {...props, ...this.DOMProps});
    }
  }
  // 转发Ref
